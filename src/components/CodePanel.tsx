...
interface CodePanelProps {
  ...
  refreshTrigger?: number; // increment to force a file tree refresh
  newlyCreatedFiles?: string[]; // paths of newly generated files to auto-open
}
...
export function CodePanel({ projectId, updatedFiles, refreshTrigger = 0, newlyCreatedFiles = [] }: CodePanelProps) {
  ...
  // Load file tree — runs on mount AND whenever refreshTrigger changes
  const loadFiles = useCallback(async (filesToOpen?: string[]) => {
    setIsLoadingTree(true);
    try {
      const fileTree = await api.getFiles(projectId);
      setFiles(fileTree);

      // If specific files to open were provided (after AI generation), open them
      if (filesToOpen && filesToOpen.length > 0) {
        const validPaths = filesToOpen.filter(p => findFileInTree(fileTree, p));
        if (validPaths.length > 0) {
          setOpenTabs(prev => {
            const merged = [...prev];
            validPaths.forEach(p => { if (!merged.includes(p)) merged.push(p); });
            return merged;
          });
          setActiveTab(validPaths[0]);
          return;
        }
      }

      // Default: open src/pages/Index.tsx if no tabs open
      setOpenTabs(prev => {
  ...
  // Refresh when refreshTrigger increments (after AI generation completes)
  useEffect(() => {
    if (refreshTrigger > 0 && refreshTrigger !== prevRefreshTriggerRef.current) {
      prevRefreshTriggerRef.current = refreshTrigger;
      loadFiles(newlyCreatedFiles.length > 0 ? newlyCreatedFiles : undefined);
    }
  }, [refreshTrigger, newlyCreatedFiles, loadFiles]);
