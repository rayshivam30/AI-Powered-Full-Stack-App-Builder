          ...
          {isStreaming && (
            <span className="text-xs text-emerald-400 flex items-center gap-1.5 font-medium animate-pulse">
              ...
              {(() => {
                ...
                const rawContent = lastMsg?.role === "assistant" ? lastMsg.content : "";
                const matches = [...rawContent.matchAll(/<file\s+path\s*=\s*["']?([^"'\s>]+)["']?>/gi)];
                if (matches.length > 0) {
                  const lastPath = matches[matches.length - 1][1];
                  return `Building ${lastPath.split('/').pop()}...`;
                }
                return "Thinking & Designing...";
              })()}
            </span>
          )}
        </div>
